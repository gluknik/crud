...
const initApp = () => {
  ...
  $edit.addEventListener('submit', e => {
    ...
    crud.methods
      ...
      .send({from: accounts[0]})
      .then(() => {
        $editResult.innerHTML = `Changed name of user ${user} to ${name}`;
      })
      .catch(() => {
        $editResult.innerHTML = `Ooops... there wazs an error while trying to update user ${id}`;
      });
  });
  ...
  $delete.addEventListener('submit', e => {
    ...
    crud.methods
      ...
      .catch(() => {
        $deleteResult.innerHTML = `Ooop... there was an error while trying delete user ${id}`;
      });
  }); 
